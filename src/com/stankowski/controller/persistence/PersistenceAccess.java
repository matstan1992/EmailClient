package com.stankowski.controller.persistence;
...
public class PersistenceAccess {
    ...
    private void encodePasswords(List<ValidAccount> persistedList) {
        for(ValidAccount validAccount: persistedList) {
            String orginalPassword = validAccount.getPassword();
            validAccount.setPassword(encoder.encode(orginalPassword));
        }
    }
    ...
    private void decodePasswords(List<ValidAccount> persistedList) {
        for(ValidAccount validAccount: persistedList) {
            String orginalPassword = validAccount.getPassword();
            validAccount.setPassword(encoder.decode(orginalPassword));
        }
    }
